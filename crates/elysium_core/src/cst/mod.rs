...
#[allow(dead_code)]
pub const PKG_NAME: &str = "elysium";

/// The publicly visible user-agent of the command-line tool
#[allow(dead_code)]
pub const SERVER_AGENT: &str = concat!("elysium ", env!("CARGO_PKG_VERSION"));
...
pub static RUNTIME_STACK_SIZE: Lazy<usize> = Lazy::new(|| {
	...
	option_env!("ELYSIUM_RUNTIME_STACK_SIZE")
...
pub static RUNTIME_MAX_BLOCKING_THREADS: Lazy<usize> = Lazy::new(|| {
	option_env!("ELYSIUM_RUNTIME_MAX_BLOCKING_THREADS")
		.and_then(|s| s.parse::<usize>().ok())
		.unwrap_or(512)
});

/// The version identifier of this build
pub static PKG_VERSION: Lazy<String> = Lazy::new(|| match option_env!("ELYSIUM_BUILD_METADATA") {
